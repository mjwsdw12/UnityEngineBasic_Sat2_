...
namespace Delegate
{
    internal class Program
    {
        ...
        // event 한정자
        // delegate 를 위한 한정자,
        // 외부 클래스에서 함수를 대리자에 등록하거나 제거할수는 있으나 직접 대리자를 호출 할 수 는 없도록 하는 한정자
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Console.WriteLine("연산을 수행하고 싶으면 DoOP 를 입력하세요");
                string input = Console.ReadLine();

                if (input == "DoOP")
                {
                    int a = 0, b = 0;
                    Console.WriteLine("첫번째 피연산를 입력하세요");
                    try
                    {
                    ...
                    {
                        // 예외 catch 가 되든 안되는 마지막에 수행하고 싶은 내용
                    }

                    Console.WriteLine("두번째 피연산를 입력하세요");
                    try
                    {
